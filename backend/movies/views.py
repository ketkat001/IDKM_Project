...
def plusweight(user_pk, movie_pk):
    ...
    user_queryset = User_tagdatas.objects.filter(user_id=user_pk).values('tagdata_id')
    tagging = []
    u_tagging = []
    ...
    for tag in tagging:
        if tag in u_tagging:
            user_tag = User_tagdatas.objects.get(user_id = user_pk, tagdata_id = tag)
            user_tag.weight += 1
            user_tag.save()
        else:
            user_tag = User_tagdatas.objects.create(user_id = user_pk, tagdata_id = tag)
            user_tag.weight += 1
            user_tag.save()
    return
